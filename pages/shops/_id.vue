...
<p><nuxt-link to="/shops/">&lt;&lt;店舗管理</nuxt-link></p>
    <h2>{{ shop_name }} の登録単価</h2>
    <hr />
...
export default {
    async asyncData(context) {
        ...
        var shop_id = parseInt(context.params.id);
        var shopdb = await ShopDB.connect();
        var shop = await shopdb.get(shop_id);

        var itemdb = await ItemDB.connect();

        var pricedb = await PriceDB.connect();
        var prices = await pricedb.getByShopId(shop_id);
        for (var i = 0; i < prices.length; i++) {
            var idx_key = prices[i].idx_key;
            prices[i].updated = context.app.$ts2date(prices[i].updated);

            var item = await itemdb.get(prices[i].item_id);
            prices[i].item_name = item.item_name;
        }
